...
TRUFOR_DIR = os.path.join(ROOT, "TruFor", "TruFor_train_test")
...
def run_mrz(image_path):
    try:
        ...
        return {
            "status": "success" if data else "not_detected",
            "data": data
        }
...
def run_trufor(image_path, output_dir, gpu):
    os.makedirs(output_dir, exist_ok=True)

    result_path = os.path.join(
        output_dir,
        os.path.basename(image_path) + ".npz"
    )
    ...
    try:
        ...
        result = {
            ...
            "map_mean": float(data["map"].mean()),
            "conf_mean": float(data["conf"].mean()),
            "npz": result_path
        }
...
def make_heatmap(image_path, npz_path, output_path):
    ...
    def screen(self):
        ...
        if document_type == "Passport":
            ...
            if mrz.get("data"):
                ...
                self.log(
                    f"Passport : {d.get('passport_number')}"
                )

                self.log(
                    f"Name     : {d.get('surname')}, {d.get('given_names')}"
                )
                ...
                self.log(
                    f"DOB      : {d.get('date_of_birth')}"
                )

                self.log(
                    f"Expiry   : {d.get('expiration_date')}"
                )
            ...
            if pan.get("data"):
                ...
                self.log(
                    f"PAN    : {d.get('pan_number')}"
                )

                self.log(
                    f"Valid  : {d.get('valid_number')}"
                )
            ...
            if aadhaar.get("data"):
                ...
                self.log(
                    f"Aadhaar : {d.get('aadhaar_number')}"
                )
        ...
        report["trufor"] = trufor

        self.log(
            f"Status : {trufor['status']}"
        )

        heatmap = None

        if trufor.get("result"):

            t = trufor["result"]

            heatmap = os.path.join(
                self.output_dir,
                os.path.basename(self.image_path) + "_heatmap.jpg"
            )
            ...
            self.log(
                f"Map Mean   : {t['map_mean']:.4f}"
            )

            self.log(
                f"Confidence : {t['conf_mean']:.4f}"
            )
